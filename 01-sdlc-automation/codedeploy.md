...
- It is a service used to automate application deployments
...
## Deploying to EC2/On-Premises Platform

- CodeDeploy supports 2 kind of deployments:
    - In-place deployments
    - Blue/Green deployments
- The instance where we want to deploy to must have the CodeDeploy Agent installed
...
- CodeDeploy with EC2 - In-place Deployments:
    - We have to use EC2 tags or ASG to identify instances where we want to deploy to
    - If we are using a load balancer, the traffic to the instance is stopped before the instance is updated and started again after the update is finished
    - Deployment hooks:
...
- CodeDeploy with EC2 - Blue/Green Deployments:
    ...
    - Instance termination:
        - `BlueInstanceTerminationOption`: we can specify whether to terminate the original EC2 instance or not after a successful deployment 
...
- CodeDeploy with On-Premise Instances:
    - Deploying with CodeDeploy to an On-Premise instance involves the following steps:
...
- Deployment traffic-shifting strategies:
    ...
    - AllAtOnce: switch immediately to the new target group
- The ECS Task Definition and the new container images must be already created before CodeDeploy executes
- Reference to the new ECS Task Revision (`TaskDefintion`) and the load balancer information (`LoadBalancerInfo`) are specified in the `appspec.yml` file
...
- CodeDeploy can help automate traffic shifting for Lambda Aliases:
    ...
    - *Linear*: traffic is shifted in equal increments, step by step, example 10% more of traffic is shifted after every 1 minute
- Traffic shifting is fully integrated with the AWS SAM framework
...
- Deployment hooks (lifecycle events):
    - **ApplicationStop**: occurs before the new revision of the application is downloaded. Used for defining a script which gracefully stops the application
...
- In case we have a load balancer, we can have the following additional hooks:
...
- Hooks which can execute scripts can have the following properties:
...
- Use cases:
    - We can use a CloudWatch event to pass a notification to a Slack channel whenever a deployment fails
...
- Triggers integrate directly to AWS SNS
...
- Deployment Error: `InvalidSignatureException` - Signature expired
    - For CodeDeploy to perform its operation, it requires accurate time references
