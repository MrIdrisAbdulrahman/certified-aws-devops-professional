...
- Use case for these events: automatically react to these events, for example generate a thumbnail for a newly uploaded image
...
- IAM permissions for Event Notifications:
    - In case of SNS: we create a resource policy for SNS where we allow `SNS:Publish` from `s3.amazon.com` service principal
    - In case of SQS: similar to SNS
    - For Lambda we define a resource access policy for the function as well

## S3 Event Notifications with EventBridge

- All S3 events will end-up in EventBridge
- With EventBridge, we can setup filters and rules for these events and connect them with all the destinations supported by EventBridge.
