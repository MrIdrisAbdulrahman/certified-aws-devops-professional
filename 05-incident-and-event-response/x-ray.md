...
- Tracing is made up of segments (+ sub segments)
...
- Instrumentation means measuring the performance of a product, diagnosing errors and writing trace information
- To instrument an application, we can use the X-Ray SDK.
- Concepts:
    - **Segments**: A segment records trace information about a request that your application serves. At a minimum, a segment records the name, ID, start time, trace ID, and end time of the request
    ...
    - **Trace**: segments and subsegments collected together to form an end-to-end trace
    ...
    - **Metadata**: key-value pairs which are not indexed, cannot be used for searching and filtering
...
- **PutTelemetryRecord**: upload telemetry record to AWS X-Ray.
      Telemetry examples: SegmentsReceivedCount, SegmentRejectedCount, BackendConnectionErrors.
- **GetSamplingRules**: retrieve all sampling rules (can be useful for X-Ray daemon to know when to send segments)
...
- **GetTraceSummaries**: retrieves the IDs and annotations for traces available for a specified timeframe using an optional filter. To get full traces, pass the trace id to BatchGetTraces API
