...
- Buckets do have a naming convention:
    ...
    - Name should not be an IP address
...
- If uploading more than 5 GB, we must use multi-part upload (recommended for more than 100 MB uploads). 
- Each object in an S3 bucket can have metadata, tags and a (optional) version ID
...
- Objects are server-side encrypted (SSE)
...
- S3 does not store the encryption key provided by the customers
- To upload data, HTTPS must be used
...
- Client-side libraries can be used to accomplish this, eg. Amazon S3 Encryption Client
- Customers are responsible for data encryption
...
- The request wouldn't be fulfilled unless the other origin allows for the request, using CORS Headers (ex: `Access-Control-Allow-Origin`, `Access-Control-Allow-Method`)
...
- S3 provides *read after write* consistency for PUTS of new objects
    - As soon as a new object is written, we can retrieve it (PUT 200 => GET 200)
    - This is true, except if we did a GET beforehand to see if the object existed (GET 404 => PUT 200 => GET 404)
- S3 provides *eventual consistency* for DELETES and PUTS of existing objects
    - If we read an object after updating it, we might get its older version (PUT 200 => PUT 200 => GET 200 (which might return the older version))
...
- It is a nice way to get protection against unintentional file encryptions (example: ransomware)
- Deleting a file is S3 bucket just adds a delete marker on the versioning
- To delete a bucket, we need to remove all files and their versions from it
...
- Bucket polices are evaluated before the default encryption flag is applied
...
- Replication can be either cross-region replication (CRR) or same-region replication SRR)
...
- For delete operations:
    - If we delete without a version ID, S3 adds a delete marker which is not replicated
    - If we delete an object with a version ID, it deletes the object from the source bucket, operation is not replicated
...
- Use cases for pre-signed URLs:
    ...
    - Temporarily allow a user to upload a file to a precise location in a bucket
- Command to generate a pre-signed url:
...
- We can restrict who can access our distribution by using:
    - Whitelists: allow our users to access our content only if they are in one of the countries on a list of approved countries
...
- S3 Cross Region Replication:
    - Must be set up for each region where we want replication to happen
...
- Used for auditing and reporting the replication and encryption status of our objects
- Use cases:
    - Business
    - Compliance
    - Regulatory needs
- We can query all the data using Amazon Athena, Redshift, Presto, Hive, Spark, etc.
- We can setup multiple inventories
- Inventory setup: we have to setup a target bucket where inventory data will be saved and define a policy for it
...
- 99.99% availability all-year-round
- Can sustain 2 concurrent facility failures
- Suitable for frequently accessed data for use cases such as: big data, mobile and gaming applications, content distribution
...
- Can sustain 2 concurrent facility failures
- Suitable for data that is less frequently accessed, but requires rapid access when needed. Use cases: data storage for disaster recovery, backups
...
- Use cases: storing secondary backup copies or data that can be recreated
...
- Expiration actions: automatically expire (delete) objects after sometime.
       - Examples:
...
- Our application can achieve atleast 3500 PUT/COPY/POST/DELETE and 5500 GET/HEAD requests per second per prefix in a bucket
...
- When we download an encrypted file, it will call the `Decrypt` KMS API
...
- Multi-Part upload:
    ...
    - Can help parallelize uploads, thus, speeding up transfers
- S3 Transfer Acceleration:
    - Increases transfer speed by transferring file(s) to an AWS edge location which will forward the data to the S3 bucket in the target region
...
- Event consumers can be:
    ...
    - SQS
    - Lambda Functions
- S3 events notifications typically deliver events in seconds, sometimes can take more than a minute
- If two writes are made to a single non-versioned object at the sametime, it is possible that only a single event notification will be sent. To solve this issue, we have to enable versioning on the bucket
...
- Provides 3 types of operations:
    ...
    - Download: first initiates a retrieval job for the particular archive, then prepares the data for download. Users have a limited time to download the data from the staging server
...
- Vault lock policies are used for regulatory and compliance requirements:
    ...
    - Example: forbid to delete an archive if it is less than 1 year old
    - Example: WORM (write once read many times) policy
...
1. Request a Snowball delivery from the AWS console 
...
- Better option than Snowball in case we want to transfer more than 10PB of data

## Snowball into Glacier

- Snowball cannot import data into Glacier directly
- We have to use an S3 bucket and create an S3 lifecycle policy to transfer data to Glacier
...
- **File Gateway**:
    ...
    - *Automating Cache Refresh*: 
        - A File Gateway feature that enables us to automatically refresh File Gateway cache to stay upto date with the changes in the S3 bucket
...
- **Tape Gateway**:
    ...
    - Backs up data using existing tape-based processes (and iSCSI interface)
