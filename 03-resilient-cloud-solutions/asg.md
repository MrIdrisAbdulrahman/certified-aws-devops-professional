...
- The goal of an Auto Scaling Group (ASG) is to:
    - Scale out (add EC2 instances) to match an increasing workload
    - Scale in (remove EC2 instances) to mach a decreasing workload
    - Ensure we have a minimum and a maximum number of machines running
    - Automatically register new instances to a Load Balancer (LB)
- ASG attributes:
...
- Both allow to specify the AMI, the instance type, a key pair, security groups and other parameters needed to launch EC2 instances (tags, user-data, etc.)
- Launch Configurations are considered to be legacy:
    - They must be recreated every time they are updated
- Launch Templates:
    ...
    - Hierarchy: templates can have parents (i.e. source templates)
    - Recommended by AWS
...
- **Scheduled scaling**:
    - Scaling based on a schedule allows us to scale the application ahead of known load changes
- **Dynamic scaling**:
...
- Slow start duration: in a target group we can set a duration (i.e. period) during which the number of requests will be gradually increased to the new instance
...
- Standby mode:
    ...
    - The load will be increased on other instances, no new instances will be created
- Scale-in protection: this can be set on for a specific instance(s). If a scale-in action happens, instances protected by scale-in protection wouldn't be terminated
...
- We can perform actions before the instance goes in service at the `Pending` state or when the instance is in the `Terminating` state
- Use cases:
    - We can run a script to download and install software when a scale-out event occurs
    - When a scale-in event happens, we can send a notification to EventBridge to execute a Lambda function in order to download logs from the instance
...
- Default termination policy: detects which AZ has the most instances with at least one instance which does not have termination protection. Within an AZ the default termination policy behavior is the following:
    ...
    3. If there are multiple instances with the latest launch configuration, terminate the one which is the closest to the next billing hour. If there are a multiple of such instances, termination is done randomly.
...
- Warm pool size settings:
    ...
    - Minimum warm pool size: static value set for the pool's minimum number of instances
- Warm pool instance state:
    - We can keep instances in the warm pool in one of these states: `Stopped`, `Running`, `Hibernated`
    - Any instance in the warm pool does not contribute to the ASG metrics that affect Scaling Policies
...
- We can create a custom metric which takes into consideration the number of messages in the queue, the number of currently running instances in the ASG and the processing time for a message from the queue. We can compute the number of instances required based on the accepted latency, example:
...
- Scale-In protection: we should protect our instances from scale-in, in case they are processing a message, we would not want the instance to be terminated
...
- `UpdatePolicy` attribute: specifies how CloudFormation handles updates:
    - `AutoScalingReplacingUpdate`: specifies whether CloudFormation replaces an Auto Scaling Group with a new one or replaces only the instances in the Auto Scaling group
    ...
    - `AutoScalingRollingUpdate`: rolling updates enable us to specify whether AWS CloudFormation updates instances that are in an Auto Scaling group are updated in batches or all-at-once
    ...
    - `AutoScalingScheduledAction`: prevents scheduled actions from modifying min/max/desired capacity for CloudFormation
...
- Replace (one LB, one TG, two ASGs, new instances): new ASG is created with a new target group and new instances
- Blue/Green (tow LB, two TGs, two ASGs, new instances, R53)
