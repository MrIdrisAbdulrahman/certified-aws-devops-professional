...
- It is an alternative to ECS, having a similar goal but with a different API
- EKS supports EC2 launch mode if we want to deploy worker nodes or Fargate to deploy serverless containers
- Use case for EKS: if our company is already using Kubernetes on-premise or on another cloud and wants to migrate to AWS
...
- We can capture node, pod and container logs and send them to CloudWatch Logs
